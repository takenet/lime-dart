import 'package:lime/protocol/types/discrete_types.dart';
import 'package:lime/protocol/types/sub_types.dart';

class MediaType {
  /// Represents the text/plain media type.
  static final MediaType textPlain = MediaType(type: DiscreteTypes.text, subtype: SubTypes.plain);

  /// Represents the application/json media type.
  static final MediaType applicationJson = MediaType(type: DiscreteTypes.application, subtype: SubTypes.json);
  ...
  bool isJson() => suffix?.toLowerCase() == SubTypes.json || subtype.toLowerCase() == SubTypes.json;

  @override
  String toString() => '$type/$subtype${(suffix?.isNotEmpty ?? false) ? '+$suffix' : ''}';
